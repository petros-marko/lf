...
Lemma f_apply_curried: forall (X Y Z: Type) (f: X -> Y -> Z) (x: X) (y: Y), f x y = Exercises.prod_uncurry f (x, y). 
...
Proof.
  ...
  induction l.
  - intros. simpl in H.
    rewrite -> f_apply_curried with (f := combine).
    rewrite <- H.
    reflexivity.
  - intros. simpl in H. destruct x, (split l).
    rewrite -> f_apply_curried with (f := combine).
    rewrite <- H.
    simpl.
    specialize IHl with (l1 := x0).
    specialize IHl with (l2 := y0).
    rewrite -> IHl.
    + reflexivity.
    + reflexivity.
  Qed.
...
Proof.
  intros.
  destruct b eqn:Arg, (f true) eqn:Ftrue, (f false) eqn:Ffalse.
...
Proof.
  ...
  - intros.
    ...
    + reflexivity.
  - intros.
    simpl. destruct m eqn:M.
    + reflexivity.
    + specialize IHn with (m := n0).
      simpl. apply IHn.  
  Qed.
(** [] *)
...
Definition split_combine_statement : Prop
  ...
  := forall (X Y: Type) (l: list (X*Y)) (lx: list X) (ly: list Y),
      length lx = length ly -> combine lx ly = l -> split l = (lx, ly).
...
Proof.
  ...
  intros X Y.
  induction l.
  - intros. destruct lx eqn:LX, ly eqn:LY.
    + reflexivity.
    + discriminate.
    + discriminate.
    + discriminate.
  - intros. destruct lx eqn:LX, ly eqn:LY.
    ...
    + discriminate.
    + discriminate.
    + specialize IHl with (lx := l0).
      specialize IHl with (ly := l1).
      simpl in H. injection H as H.
      simpl in H0. injection H0 as H0. symmetry in H0.
      rewrite H0.
      apply IHl in H.
...
Proof.
  ...
  - discriminate H.
  - simpl in H.
    destruct (test x0) eqn:T in H.
    + injection H as H'. rewrite H' in T. apply T.
