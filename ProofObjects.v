...
Definition or_commut' : forall P Q, P \/ Q -> Q \/ P :=
  fun _ _ HPQ =>
    match HPQ with
    | or_introl HPQ' => or_intror HPQ'
    | or_intror HPQ' => or_introl HPQ'
    end.
(** [] *)
...
Definition equality__leibniz_equality_term : forall (X : Type) (x y: X),
    x == y -> forall P : (X -> Prop), P x -> P y :=
    fun X x y (E : @eq X x y) (P : X -> Prop) =>
      match E with
      | eq_refl z => fun evP => evP
      end.
(** [] *)
...
Definition or_distributes_over_and_forward : forall P Q R: Prop,
  P \/ (Q /\ R) -> (P \/ Q) /\ (P \/ R) :=
  fun _ _ _ HPQR =>
    match HPQR with
...
Definition or_distributes_over_and_backward : forall P Q R: Prop,
  (P \/ Q) /\ (P \/ R) -> P \/ (Q /\ R) :=
  fun _ _ _ H_PQ_PR =>
    match H_PQ_PR with
...
Definition or_distributes_over_and : forall P Q R : Prop,
    P \/ (Q /\ R) <-> (P \/ Q) /\ (P \/ R) :=
    fun P Q R => 
      conj ( or_distributes_over_and_forward P Q R ) ( or_distributes_over_and_backward P Q R ).
(** [] *)

(** **** Exercise: 3 stars, standard (negations) *)
Definition double_neg : forall P : Prop,
    P -> ~~P := fun _ HP HNP => HNP HP.
...
Definition de_morgan_not_or : forall P Q : Prop,
    ...
    fun _ _ HNPQ =>
      conj (
        fun HP => match HNPQ (or_introl HP) with end
      ) (
        fun HQ => match HNPQ (or_intror HQ) with end
      ).
(** [] *)
...
Definition curry : forall P Q R : Prop,
    ((P /\ Q) -> R) -> (P -> (Q -> R)) :=
    fun _ _ _ fPQR HP HQ => fPQR (conj HP HQ).

Definition uncurry : forall P Q R : Prop,
    (P -> (Q -> R)) -> ((P /\ Q) -> R) :=
    fun _ _ _ fPQR HPQ =>
      match HPQ with | conj HP HQ => fPQR HP HQ end.
(** [] *)
...
Proof.
  intros H P HP.
  apply H.
  apply provable_equiv_true in HP. symmetry in HP.
  apply HP.
  Qed.
(** [] *)
...
Proof.
  ...
  apply (pe_implies_true_eq H) in pf1 as T1.
  subst. destruct pf1, pf2.
  reflexivity.
  Qed .
(** [] *)
