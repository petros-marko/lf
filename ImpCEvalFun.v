...
Proof.
  intros c st st' Hce.
  induction Hce;
  try (exists 1; simpl; try rewrite H; reflexivity);
  try (destruct IHHce; exists (S x); simpl; rewrite H; apply H0).
  - destruct IHHce1, IHHce2.
    exists (S (x + x0)). simpl.
    rewrite (ceval_step_more x (x + x0) st st' c1).
    apply (ceval_step_more x0 (x + x0) st' st'' c2).
  ...
  - destruct IHHce1, IHHce2.
    ...
    rewrite (ceval_step_more x (x + x0) st st').
    apply (ceval_step_more x0 (x + x0) st' st'').
    + rewrite add_comm. apply le_add_r.
    + assumption.
    + apply le_add_r.
    + assumption.
