...
Theorem Perm3_refl : forall X a b c, @Perm3 X [a; b; c] [a; b; c].
Proof.
  intros.
  apply (perm3_trans [a; b; c] [b; a; c] [a; b; c]).
  - apply perm3_swap12.
  - apply perm3_swap12.
  Qed.

Example Perm3_123_refl : Perm3 [1;2;3] [1;2;3].
Proof. apply Perm3_refl. Qed.
...
Lemma double_dist_plus: forall (n m : nat), double n + double m = double (n + m).
Proof.
  intros.
  induction n.
  - reflexivity.
...
Proof.
  intros.
  apply ev_Even_iff in H. unfold Even in H. destruct H.
  apply ev_Even_iff in H0. unfold Even in H0. destruct H0.
  rewrite ev_Even_iff. unfold Even.
  exists (x + x0).
  rewrite H. rewrite H0.
  apply double_dist_plus.
  Qed.
(** [] *)
...
Proof.
  ...
  assert (R: ((n + m) + (n + p)) = (double n) + (m + p)).
    { rewrite double_plus. 
      rewrite add_shuffle3. 
      rewrite add_assoc. rewrite add_assoc. rewrite (add_assoc (n + n) m p). reflexivity.
    }
  rewrite R in S.
  apply (ev_ev__ev (double n) (m + p)) in S.
  - apply S.
...
Proof.
  ...
  split.
  {
    apply (le_trans n1 (n1 + n2) m) in H.
    - apply H.
    - apply le_plus_l. 
  }
  {
    rewrite add_comm in H.
    apply (le_trans n2 (n2 + n1) m) in H.
    - apply H.
    - apply le_plus_l.
  }
  Qed.
...
Proof.
  ...
  - intros.
    ...
    { 
      induction p.
      - right. simpl in H. apply (le_trans m (S (n + m)) q).
        + rewrite add_comm. rewrite plus_n_Sm. apply le_plus_l.
        + apply H.
      - apply (Sn_le_Sm__n_le_m (n + m) (p + q)) in H.
        apply IHn in H.
        destruct H.
...
Proof.
  ...
  - intros. apply IHp in H.
    apply (n_le_m__Sn_le_Sm (p + n) (p + m)).
    apply H.
  Qed.
...
Proof.
  ...
  induction m.
  - intros. destruct n. apply le_n. discriminate H.
  - intros. destruct n.
    + apply O_le_n.
...
Proof.
  ...
  - intros. inversion H.
    + apply leb_refl.
    + apply IHm in H1.
      apply leb_plus_exists in H1. destruct H1.
      apply plus_exists_leb.
      exists (S x). rewrite <- plus_n_Sm. 
...
Proof.
  ...
  apply leb_iff in H. apply H.
  apply leb_iff in H0. apply H0.
  Qed.
(** [] *)
...
Inductive subseq : list nat -> list nat -> Prop :=
  ...
  | subseq_cons (n : nat) (l1 l2 : list nat) (H : subseq l1 l2): subseq (n :: l1) (n :: l2)
  | subseq_add_r (n : nat) (l1 l2 : list nat) (H : subseq l1 l2): subseq l1 (n :: l2)
  .
...
Proof.
  ...
  induction H.
  - apply subseq_nil.
  - simpl. apply subseq_cons.
    apply IHsubseq.  
  - simpl. apply subseq_add_r.
    apply IHsubseq.
  Qed.
...
Proof.
  ...
  - intros. inversion T12.
    + apply subseq_nil.
    + apply subseq_cons. apply IHT23. apply H1.
    + apply subseq_add_r. apply IHT23. apply H1.
  - intros. apply subseq_add_r. apply IHT23. apply T12.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. apply MStarApp.
    + apply (H x). unfold In. left. reflexivity.
    + apply IHss.
      intros.
      assert (H': In s (x :: ss)).
...
Proof.
  ...
  - apply IHexp_match2 in Heqre' as H'.
    injection Heqre' as Re0Re.
    destruct H'. destruct H1.
    exists ([s1] ++ x). apply conj.
    + simpl. rewrite <- H1. reflexivity.
    + intros. destruct H3.
      * rewrite <- H3. rewrite <- Re0Re. apply H.
      * apply H2. apply H3.
  Qed.
(** [] *)
...
Inductive sorted: list nat -> Prop :=
  | sorted_nil : sorted []
  | sorted_single (n : nat) : sorted [n]
  | sorted_cons (n x : nat) (xs : list nat) (Hle: n <= x) (Hs: sorted (x :: xs)) : sorted (n :: x :: xs).
Fixpoint insert (n : nat) (ns : list nat) :=
  match ns with
  | [] => [n]
...
Theorem sorted_drop : forall n ns, sorted (n :: ns) -> sorted ns.
Proof.
  intros. remember (n :: ns) as l.
  induction H.
  - discriminate Heql.
  - inversion Heql. apply sorted_nil.
  - injection Heql as Heql.
    rewrite <- H0.
    apply H.
  Qed.
Theorem leb_antisym : forall m n, m <=? n = false -> n <=? m = true.
Proof.
  induction m.
  ...
  - intros.
    simpl in H.
    destruct n.
    + apply leb_iff. apply O_le_n.
    + apply IHm in H.
      apply leb_iff in H. apply n_le_m__Sn_le_Sm in H.
      apply leb_iff. apply H.
  Qed.
Lemma insert_head_cases : forall x n ns y ys, (insert x (n :: ns) = y :: ys) -> x = y \/ n = y.
Proof.
  intros.
  unfold insert in H.
  destruct (x <=? n) eqn:XN.
  - left. injection H as H. apply H.
  - right. injection H as H. apply H.
  Qed.
Lemma insert_nonempty : forall x ns, exists y ys, insert x ns = y :: ys.
Proof.
  intros.
  destruct ns.
  - exists x, []. reflexivity.
  - destruct (x <=? x0) eqn:X.
    + simpl. rewrite X. exists x, (x0 :: ns). reflexivity.
    + simpl. rewrite X. exists x0, (insert x ns). reflexivity.
  Qed.
Theorem insert_in_sorted_sorted : forall x l, sorted l -> (sorted (insert x l)).
Proof.
  intros.
  induction H.
  - simpl. apply sorted_single.
  - destruct (x <=? n) eqn:XN.
    + simpl. rewrite XN. apply sorted_cons.
      * apply leb_iff. apply XN.
      * apply sorted_single.
    + simpl. rewrite XN. apply sorted_cons.
      * apply leb_iff. apply leb_antisym in XN. apply XN.
      * apply sorted_single.
  - destruct (x <=? n) eqn:XN.
    + simpl. rewrite XN. apply sorted_cons.
      * apply leb_iff. apply XN.
      * apply sorted_cons.
        -- apply Hle.
        -- apply H.
    + replace (insert x (n :: x0 :: xs)) with (n :: insert x (x0 :: xs)).
      {
        destruct (insert x (x0 :: xs)) as [|y ys] eqn:I.
        - apply sorted_single.
        - apply insert_head_cases in I.
          destruct I.
          + rewrite <- H0. rewrite <- H0 in IHsorted.
            apply sorted_cons.
            * apply leb_iff. apply leb_antisym in XN. apply XN.
            * apply IHsorted.
          + rewrite <- H0. rewrite <- H0 in IHsorted.
      ...
      { simpl. rewrite XN. reflexivity. }
  Qed.
Theorem insertion_sort_sorts : forall l, sorted (insertion_sort l).
Proof.
  intros.
  induction l.
  - simpl. apply sorted_nil.
  - simpl. 
    apply insert_in_sorted_sorted.
    apply IHl.
  Qed.
End InsertionSort.
...
Proof.
  ...
  - intros. 
    ...
    destruct H.
    + apply IH1 in H. destruct H as [x [y [z]]].
      destruct H as [S1 [Y P]].
      exists x, y, (z ++ s2).
      split.
      {
        rewrite S1. rewrite <- app_assoc. rewrite <- app_assoc.
        reflexivity.
      }
      {
        split. apply Y.
        {
          ...
          rewrite app_assoc.
          apply MApp.
          - rewrite <- app_assoc. apply P.
          - apply Hmatch2.
        }
      }
    + apply IH2 in H. destruct H as [x [y [z]]].
      destruct H as [S2 [Y P]].
      exists (s1 ++ x), y, z.
      split.
      {
        rewrite <- app_assoc. rewrite S2.
        reflexivity.
      }
      {
        split. apply Y.
  ...
  - intros. simpl in H. rewrite <- add_0_r in H.
      apply add_le_cases in H. destruct H.
      + apply IH in H. destruct H as [x [y [z]]].
        destruct H as [S1 [Y P]].
        exists x, y, z.
        split.
          { apply S1. }
      ...
      + inversion H.
        ...
        destruct H1.
  - intros. simpl in H. rewrite <- add_0_r in H. rewrite add_comm in H.
      apply add_le_cases in H. destruct H.
      + apply IH in H. destruct H as [x [y [z]]].
        destruct H as [S1 [Y P]].
        exists x, y, z.
  ...
  - intros. rewrite app_length in H.
    destruct s2 eqn:S2.
    {
      ...
      exists x, y, z.
      split.
      { rewrite app_nil_r. apply S1. }
      { 
        split.
        { apply Y. }
        {
          intros m.
          apply MStar1. apply P.
        }
      }
    }
    {
      exists s1, s2, [].
      split.
      { rewrite <- S2. rewrite app_assoc. rewrite app_nil_r. reflexivity. }
      {
        split.
        { unfold not. intros. rewrite S2 in H0. discriminate H0. }
        {
          intros. rewrite app_nil_r.
          apply MStarApp.
          - apply Hmatch1.
          - apply star_repeats. rewrite S2. apply Hmatch2.
        }
      }
    }
  Qed.
...
Proof.
  ...
  split.
  {
    inversion H. intros. reflexivity.
...
Proof.
  ...
  - simpl in Hcount. destruct (eqbP n m).
    ...
    + simpl in Hcount. apply IHl' in Hcount.
      simpl. unfold not. intros [MN | INL].
      * symmetry in MN. apply H in MN. apply MN.
      * apply Hcount in INL. apply INL.
  Qed.
(** [] *)
...
Inductive nostutter {X:Type} : list X -> Prop :=
  ...
  | nostutter_cons (x y : X) (ys : list X) (Hneq : x <> y) (Hrec : nostutter (y :: ys)) : nostutter (x :: y :: ys)
 (* FILL IN HERE *)
.
...
Inductive merge {X:Type} : list X -> list X -> list X -> Prop :=
  | merge_nil : merge [] [] []
  | merge_cons_l (x : X) (xs ys zs : list X) (H : merge xs ys zs) : merge (x :: xs) ys (x :: zs)
  | merge_cons_r (y : X) (xs ys zs : list X) (H : merge xs ys zs) : merge xs (y :: ys) (y :: zs)
(* FILL IN HERE *)
.

Example merge_ex_1 : merge [1; 2; 3] [4; 5; 6] [1 ; 4; 5; 2; 3; 6].
Proof.
  apply merge_cons_l. apply merge_cons_r.
  apply merge_cons_r. apply merge_cons_l.
  apply merge_cons_l. apply merge_cons_r.
  apply merge_nil.
  Qed.
...
Proof.
  ...
  - reflexivity.
  - simpl. inversion H0.
    rewrite H2. apply f_equal.
    apply IHmerge in H3.
    + apply H3.
    + apply H1.
  - simpl. inversion H1.
    rewrite H2.
    apply IHmerge in H0 as H'.
    apply IHmerge in H3.
    + apply H3.
    + apply H0.
    + apply H3.
...
(* FILL IN HERE
    ...
    [] *)
Theorem filter_longest : forall l l' test, subseq l' l -> All (fun x => test x = true) l' -> length l' <= length (filter test l).
Proof.
  intros. induction H.
  - simpl. apply O_le_n.
  - simpl. inversion H0.
  ...
  - apply IHsubseq in H0.
    destruct (test n) eqn:N.
    + simpl. rewrite N.
      apply (le_trans (length l1) (length (filter test l2)) (S (length (filter test l2)))).
      * apply H0.
      * apply le_S. reflexivity.
    + simpl. rewrite N. apply H0.
  Qed.
...
Inductive pal {X:Type} : list X -> Prop :=
  ...
  | pal_ext (x : X) (p : list X) (Hp : pal p) : pal ((x :: p) ++ [x])
(* FILL IN HERE *)
.
...
Lemma length_0_empty : forall (X: Type) l, @length X l = 0 -> l = [].
Proof.
  intros. destruct l.
  - reflexivity.
  - discriminate H.
...
Lemma app_empty_empty : forall (X: Type) (xs ys : list X), xs ++ ys = [] -> xs = [] /\ ys = [].
Proof.
  induction xs.
  - simpl.
    intros. split. reflexivity. apply H.
  - intros. split. discriminate H. discriminate H.
  Qed.
...
Theorem palindrome_converse_length : forall X  n (l:list X), length l <= n -> 
l = rev l -> pal l.
Proof. intros X. induction n as [|n].
  ...
  - rewrite H in Hrev. simpl in Hrev. rewrite rev_app_distr in Hrev. 
    ...
    simpl in Hlength. rewrite app_length, add_comm in Hlength.
...
Proof.
intros.
  apply (palindrome_converse_length X (length l) l).
  - reflexivity.
  - apply H.
  Qed.
(** [] *)
...
Inductive NoDup (X: Type) : list X -> Prop :=
  | no_dup_nil : NoDup X (nil : list X)
  | no_dup_cons (x : X) (xs : list X) (Hnd : NoDup X xs) (Hnx : ~ In x xs) : NoDup X (x :: xs).

Example no_dup_ex_1 : NoDup nat [1;2;3].
Proof.
  apply no_dup_cons.
  { apply no_dup_cons.
    { apply no_dup_cons.
      apply no_dup_nil.
      unfold not. unfold In. intros. destruct H.
    }
    { 
      unfold not. unfold In. intros [TeT | F].
      discriminate TeT. destruct F.
    }
  }
  {
    unfold not. unfold In. intros [TwEO | [ThEO | F]].
    discriminate TwEO. discriminate ThEO. destruct F.
  }
  Qed.
(** Finally, state and prove one or more interesting theorems relating
    [disjoint], [NoDup] and [++] (list append).  *)

Lemma disjoint_l_drop : forall X x l1 l2, @disjoint X (x :: l1) l2 -> @disjoint X l1 l2.
Proof.
  induction l1.
  - intros. reflexivity.
  - intros.
    unfold disjoint, All in H.
    destruct H as [Ninx [Ninx0 NinR]].
    unfold disjoint.
    split.
    + apply Ninx0.
    + apply NinR. 
...
Lemma not_in_app : forall X x l1 l2, ~ @In X x l1 -> ~ In x l2 -> ~ In x (l1 ++ l2).
Proof.
  induction l1.
  - intros. simpl. apply H0.
  - intros. unfold not. intros.
    simpl in H. apply de_morgan_not_or in H. destruct H as [XneX0 NinXL1].
    simpl in H1. destruct H1 as [XeX0 | InxL1L2].
    + apply XneX0 in XeX0. destruct XeX0.
    + assert (F: ~ In x (l1 ++ l2)).
        { apply IHl1. apply NinXL1. apply H0. }
      apply F in InxL1L2. destruct InxL1L2.
  Qed.


Theorem disjoint_app_disjoint_no_dup : forall X l1 l2, @disjoint X l1 l2 -> NoDup X l1 -> NoDup X l2 -> NoDup X (l1 ++ l2).
Proof.
  induction l1.
  - intros. apply H1.
  - intros. simpl.
    ...
    apply no_dup_cons.
    apply disjoint_l_drop in H. apply IHl1 in H.
    apply H. apply Hnd. apply H1.
    apply not_in_app. apply Hnx. apply H2.
  Qed.
(* FILL IN HERE *)
...
Inductive repeats {X:Type} : list X -> Prop :=
  | repeats_x (x : X) : repeats [x; x]
  | repeats_new (x : X) (l : list X) (H : In x l) : repeats (x :: l)
  | repeats_nonew (x : X) (l : list X) (H : repeats l) : repeats (x :: l).
...
Lemma in_reorder : forall X y x l1 l2, @In X y (l1 ++ x :: l2) <-> @In X y (x :: (l1 ++ l2)).
Proof.
  intros.
  split.
  {
  ...
  }
  {
    ...
    induction l1.
    - apply H.
    - simpl in H. destruct H as [X' | [X0 | R]].
      + right. apply IHl1. left. apply X'.
      + left. apply X0.
      + right. apply IHl1. right. apply R.
  }
  Qed.
...
Proof.
  ...
  - intros.
    assert (InOrOut : In x l1' \/ ~ In x l1').
      { apply EM. }
    destruct InOrOut.
    { apply repeats_new. apply H1. }
    {
      apply repeats_nonew.
      (* 
        split up l2 into xs1 ++ [x] ++ xs2.
         notice that forall x0, In x0 l1 -> In x0 (xs1 ++ xs2). (since x is not in l1')
         also notice that S(length (xs1 ++ xs2)) = length l2 -> length (xs1 ++ xs2) < length l1'
         use IHl1' to prove the result
      *)
      assert (XinL2: In x l2).
      { apply H. left. reflexivity. }
      apply in_split in XinL2. destruct XinL2 as [xs1 [xs2]].
      assert (F: forall x0 : X, In x0 l1' -> In x0 (xs1 ++ xs2)).
        {
          intros. induction xs1.
          - simpl. simpl in H2.
            assert (X0inL2: In x0 l2).
              { apply H. right. apply H3. }
            rewrite H2 in X0inL2. simpl in X0inL2. destruct X0inL2.
            + rewrite <- H4 in H3. apply H1 in H3. destruct H3.
            + apply H4.
          - simpl.
            assert (X0inL2: In x0 l2).
            { apply H. right. apply H3. }
            rewrite H2 in X0inL2.
            rewrite in_reorder in X0inL2.
            simpl in X0inL2. destruct X0inL2 as [XisX0 | [XisX1 | XisNot]].
            + rewrite <- XisX0 in H3. apply H1 in H3. destruct H3.
            + left. apply XisX1.
            + right. apply XisNot.
        }
      assert (L: (length l2) = S (length (xs1 ++ xs2))).
        {
          rewrite H2.
      ...
      rewrite L in H0. simpl in H0. apply Sn_le_Sm__n_le_m in H0.
      apply (IHl1' (xs1 ++ xs2)).
      - apply F.
      - apply H0.
    }
...
Proof.
  intros. split.
  {
    ...
    - left. split.
      apply H2.
      simpl in H1. rewrite H1. apply H3.
...
Proof.
  intros. split.
  {
    intros. remember (a :: s) as l. remember (Star re) as r.
    induction H.
    - discriminate Heql.
    - discriminate Heqr.
    - discriminate Heqr.
    ...
    - destruct s1.
      + simpl in Heql.
        apply IHexp_match2 in Heql.
        * apply Heql.
        * apply Heqr.
      + inversion Heql. exists s1, s2. split.
        * reflexivity.
        * injection Heqr as Heqr. rewrite Heqr in H.
          split.
          rewrite <- H2. apply H.
          apply H0.
  }
  {
    ...
    rewrite H1. apply (MStarApp (a :: s0)).
    - apply H2.
    - apply H3.
  }
  Qed.
...
Proof.
  ...
  - simpl.
    ...
    + inversion IHre2.
      * apply ReflectT.
        apply (MApp []). apply H0. apply H2.
      * apply ReflectF. unfold not. intros. inversion H3.
        apply app_empty_empty in H4. destruct H4. rewrite H9 in H8. apply H2 in H8. destruct H8.
    + apply ReflectF. unfold not. intros. inversion H1.
      apply app_empty_empty in H2. destruct H2. rewrite H2 in H5. apply H0 in H5. destruct H5.
  - simpl.
    inversion IHre1.
    + apply ReflectT. apply MUnionL. apply H0.
    + inversion IHre2.
...
Fixpoint derive (a : ascii) (re : reg_exp ascii) : reg_exp ascii :=
  ...
  | EmptySet => EmptySet
  | EmptyStr => EmptySet
  | Char x => if Ascii.eqb x a then EmptyStr else EmptySet
  | App re1 re2 => if (match_eps re1) then Union (derive a re2) (App (derive a re1) re2)  
...
Lemma matches_empty_match_eps : forall re, [] =~ re -> match_eps re = true.
Proof.
  intros. induction re.
  - inversion H.
  - reflexivity.
  - inversion H.
  - inversion H.
    apply app_empty_empty in H1. destruct H1.
    rewrite H1 in H3. rewrite H5 in H4. apply IHre1 in H3. apply IHre2 in H4.
    simpl. rewrite H3. rewrite H4. reflexivity.
  - simpl. inversion H.
    + apply IHre1 in H2. rewrite H2. reflexivity.
    + rewrite orb_com.
      apply IHre2 in H1. rewrite H1. reflexivity.
  - reflexivity.
  Qed.
  
Lemma derive_corr : derives derive.
Proof.
  unfold derives. unfold is_der. intros a re.
  induction re.
  - intros. split.
    { intros. inversion H. }
    { intros. inversion H. }
  ...
  - intros. split.
    { intros. inversion H.
      simpl. rewrite Ascii.eqb_refl. apply MEmpty.
    }
    { intros.
      simpl in H.
      destruct (Ascii.eqb t a) eqn:E.
      - rewrite Ascii.eqb_eq in E. rewrite E.
        inversion H. apply MChar.
      - inversion H. 
    }
  - intros. split.
    { intros. apply app_ne in H.
      destruct H as [[E R] | [s0 [s1 [S [L R]]]]].
      - simpl. apply matches_empty_match_eps in E. rewrite E.
        apply MUnionL. apply IHre2 in R. apply R.
      - simpl. destruct (match_eps_refl re1).
        + apply MUnionR. apply IHre1 in L.
          rewrite S. apply MApp. apply L. apply R.
        + apply IHre1 in L. rewrite S. apply MApp.
          apply L. apply R.
    }
    { intros. simpl in H.
      destruct (match_eps_refl re1).
      - inversion H.
        + apply IHre2 in H3. apply (MApp []). apply H0. apply H3.
        + apply app_exists in H3. destruct H3 as [s0 [s1 [S [S0 S1]]]].
          apply IHre1 in S0. rewrite S. apply (MApp (a :: s0)).
          apply S0. apply S1.
      - apply app_exists in H. destruct H as [s0 [s1 [S [S0 S1]]]].
        apply IHre1 in S0. rewrite S. apply (MApp (a :: s0)).
        apply S0. apply S1.
    }
  - intros. split.
    { intros. simpl. apply union_disj in H. destruct H as [L | R].
      - apply MUnionL. apply IHre1. apply L.
      - apply MUnionR. apply IHre2. apply R.
    }
    { intros. inversion H.
  ...
  - intros. split.
    { intros. simpl. apply star_ne in H.
      destruct H as [s0 [s1 [S [L R]]]].
      rewrite S. apply MApp. apply IHre. apply L. apply R.
    }
    { intros. simpl in H. apply app_exists in H.
      destruct H as [s0 [s1 [S [L R]]]].
      rewrite S. apply (MStarApp (a :: s0)).
      - apply IHre. apply L.
      - apply R.
    }
  Qed.
...
Fixpoint regex_match (s : string) (re : reg_exp ascii) : bool :=
  match s with
  | [] => match_eps re
  | c :: cs => regex_match cs (derive c re)
  end.
(** [] *)

Example regex_match_ex_1 : regex_match [c ; d] (App (Char c) (Char d)) = true.
Proof. reflexivity. Qed.

Example regex_match_ex_2 : regex_match [c ; d] (App (Union (Char c) EmptyStr) (Union (Char c) (Char d))) = true.
Proof. reflexivity. Qed.
...
Proof.
  ...
  induction s.
  - intros. simpl. apply match_eps_refl.
  - intros. simpl. destruct (derive_corr x re s).
    destruct (IHs (derive x re)).
    + apply ReflectT. apply H0. apply H1.
    + apply ReflectF. unfold not. intros.
      apply H in H2. apply H1 in H2. apply H2. 
  Qed.
(** [] *)
