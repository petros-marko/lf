...
Fixpoint optimize_0plus_b (b : bexp) : bexp :=
  ...
  | BEq lhs rhs => BEq (optimize_0plus lhs) (optimize_0plus rhs)
  | BNeq lhs rhs => BNeq (optimize_0plus lhs) (optimize_0plus rhs)
  | BLe lhs rhs => BLe (optimize_0plus lhs) (optimize_0plus rhs)
  | BGt lhs rhs => BGt (optimize_0plus lhs) (optimize_0plus rhs)
  end.
...
Proof.
  ...
  induction b;
  try reflexivity;
  try (simpl; rewrite optimize_0plus_sound, optimize_0plus_sound; reflexivity).
...
Inductive bevalR: bexp -> bool -> Prop :=
  ...
  | E_BNot be bv :
      be ==>b bv -> bevalR (BNot be) (negb bv)
  | E_BAnd be1 be2 bv1 bv2 :
      be1 ==>b bv1 ->
      be2 ==>b bv2 ->
      bevalR (BAnd be1 be2) (bv1 && bv2)
where "e '==>b' b" := (bevalR e b) : type_scope
.

Lemma negb_flip : forall b1 b2, negb b1 = b2 <-> b1 = negb b2.
Proof.
  intros. split.
  - intros. destruct H. rewrite negb_involutive. reflexivity.
  - intros. inversion H. rewrite negb_involutive. reflexivity.
  Qed.
...
Inductive no_whilesR: com -> Prop :=
  ...
  | no_whiles_asgn s v : no_whilesR <{ s  := v }>
  | no_whiles_seq c1 c2 : no_whilesR c1 -> no_whilesR c2 -> no_whilesR <{ c1 ; c2}>
  | no_whiles_if b c1 c2 : no_whilesR c1 -> no_whilesR c2 -> no_whilesR <{ if b then c1 else c2 end }>
.
...
Proof.
  intros. split.
  {
    intros. induction c;
    try constructor;
    try (simpl in H; rewrite andb_true_iff in H; destruct H);
    try (apply IHc1; apply H);
    try (apply IHc2; apply H0).
    inversion H.
  }
  {
    intros. induction H;
    try (simpl; rewrite IHno_whilesR1, IHno_whilesR2);
    reflexivity.
  }
  Qed.
(** [] *)
...
Theorem no_whiles_terminating : forall p, no_whilesR p -> forall st, exists st', st =[p]=> st'.
Proof.
  intros p H.
  induction H; subst; intros st.
  - exists st. constructor.
  - exists (s !-> (aeval st v) ; st).
    constructor. reflexivity.
...
Proof.
  induction e; intros;
  try (simpl; repeat rewrite execute_app; rewrite IHe1, IHe2; simpl);
...
Inductive ceval : com -> state -> result -> state -> Prop :=
  ...
  | E_Seq_fbr : forall c1 c2 st st',
      st =[ c1 ]=> st' / SBreak ->
      st =[ c1 ; c2 ]=> st' / SBreak
  | E_Seq_sbr : forall c1 c2 st st' st'',
      st  =[ c1 ]=> st' / SContinue ->
      st' =[ c2 ]=> st'' / SBreak ->
      st  =[ c1 ; c2 ]=> st'' / SBreak
  | E_Seq_nbr : forall c1 c2 st st' st'',
      st  =[ c1 ]=> st' / SContinue ->
      st' =[ c2 ]=> st'' / SContinue ->
  ...
  | E_WhileTrue_br : forall b st st' c,
      beval st b = true ->
      st =[ c ]=> st' / SBreak ->
      st =[ while b do c end ]=> st' / SContinue
  | E_WhileTrue : forall st st' st'' b c,
...
Theorem while_stops_on_break : forall b c st st',
  ...
  st =[ while b do c end ]=> st' / SContinue.
Proof.
  intros. apply E_WhileTrue_br; assumption.
  Qed.
...
Theorem seq_continue : forall c1 c2 st st' st'',
  ...
  st =[ c1 ; c2 ]=> st'' / SContinue.
Proof.
  intros. apply (E_Seq_nbr _ _ _ st'); assumption.
  Qed.
...
Theorem seq_stops_on_break : forall c1 c2 st st',
  ...
  st =[ c1 ; c2 ]=> st' / SBreak.
Proof.
  intros. apply E_Seq_fbr; assumption.
  Qed.
(** [] *)
...
Proof.
  ...
  induction H; subst; try inversion Heqw; subst.
...
Proof.
  ...
  induction H; intros.
  - inversion H. subst; apply conj; reflexivity.
  - inversion H0; subst; apply conj; reflexivity.
  - inversion H0; subst;
    try (apply IHceval in H6; assumption);
    try (apply IHceval in H3; destruct H3; inversion H2).
  - inversion H1; subst.
    + apply IHceval1 in H7. destruct H7. inversion H3.
    + apply IHceval1 in H4. destruct H4. rewrite <- H2 in H8. apply IHceval2 in H8. assumption.
    + apply IHceval1 in H4. destruct H4. rewrite <- H2 in H8. apply IHceval2 in H8. destruct H8. inversion H5.
  - inversion H1; subst.
    + apply IHceval1 in H7. destruct H7. inversion H3.
    + apply IHceval1 in H4. destruct H4. rewrite <- H2 in H8. apply IHceval2 in H8. destruct H8. inversion H5.
    + apply IHceval1 in H4. destruct H4. rewrite <- H2 in H8. apply IHceval2 in H8. assumption.
  ...
  - inversion H1; subst.
    ...
    + apply IHceval in H8. destruct H8. apply conj.
      * assumption.
      * reflexivity.
  ...
  - inversion H2; subst.
    ...
    + apply IHceval1 in H6. destruct H6. rewrite <- H3 in H10. apply IHceval2 in H10. assumption.
  Qed.
