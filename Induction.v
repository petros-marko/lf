...
Proof.
  ...
  - simpl. 
    rewrite <- IHn. rewrite <- mult_n_Sm. rewrite -> add_comm. 
    reflexivity.
  Qed.
(** [] *)
...
Proof.
  ...
  - rewrite <- mult_n_Sm.
    ...
    assert (H3: m * p + m = m + p * m).
      { rewrite -> add_comm. rewrite -> mul_comm. reflexivity. }
    assert (H4: n * p + m * p + m = p * n + m + p * m).
      { rewrite -> mul_comm. rewrite <- add_assoc. rewrite H3. rewrite <- add_assoc. reflexivity. }
    rewrite -> H4.
    rewrite -> add_assoc.
    rewrite -> add_assoc.
    rewrite -> add_assoc.
...
Example normalize_z_z : normalize Z = Z.
Proof. reflexivity. Qed.

Example normalize_leading_zeros : normalize (B0 (B0 (B0 Z)))  = Z.
Proof. reflexivity. Qed.

Example normalize_non_leading_zeros : normalize (B0 (B0 (B1 Z))) = (B0 (B0 (B1 Z))).
Proof. reflexivity. Qed.
...
Lemma nat_to_bin_double_swap : forall n, nat_to_bin (double n) = double_bin (nat_to_bin n).
Proof.
  intros.
  induction n.
  - reflexivity.
...
Proof.
  ...
  - simpl.
    rewrite -> add_0_r.
    rewrite <- double_plus.
    rewrite nat_to_bin_double_swap.
    rewrite IHb.
    reflexivity.
  - simpl.
    rewrite -> add_0_r.
