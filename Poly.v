...
Definition zip {X Y : Type}:= @combine X Y.
...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  match l with
  | []             => ([], [])
  | (x, y) :: rest => match split rest with | ( xs, ys ) => (x :: xs, y :: ys)end
  end.
...
Lemma map_app_dist : forall (X Y : Type) (f : X -> Y) (l1 l2 : list X), map f (l1 ++ l2) = map f l1 ++ map f l2.
Proof.
  intros.
  induction l1.
  - reflexivity.
...
Example fold_map_test1: fold_map (fun x => S x) [1 ; 2 ; 3] = [2 ; 3 ; 4].
Proof. reflexivity. Qed.
