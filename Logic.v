...
Proof.
  intros.
  destruct n eqn:N.
  - simpl in H. rewrite H. apply conj.
    + reflexivity.
    + reflexivity.
  - discriminate H.
  Qed.
...
Lemma prop_trans: forall (P Q R: Prop), ((P -> Q) /\ (Q -> R)) -> P -> R.
Proof.
  intros.
  destruct H.
  apply H in H0.
...
Proof.
  ...
  unfold not in H0.
  unfold not.
  apply prop_trans with (Q := Q) (R := False).
  apply conj.
  - apply H.
  - apply H0.
  Qed.
...
Proof.
  ...
  apply conj.
  - intros.
    assert (P \/ Q). { left. apply H0. }
    apply H in H1.
    apply H1.
  - intros.
    assert (P \/ Q). { right. apply H0. }
    apply H in H1.
    apply H1.
  Qed.
(** [] *)
...
Lemma leb_n_plus : forall (n x: nat), n <=? n + x = true.
Proof.
  induction n.
  - reflexivity.
  - intros. simpl.
    specialize IHn with (x := x).
    apply IHn.
  Qed.
...
Proof.
  ...
  { induction l as [|x l' IHl']. 
    ...
    - simpl. intros. destruct H.
      + exists x.
        apply conj. apply H. left. reflexivity.
      + apply IHl' in H. destruct H.
        exists x0.
        apply conj. apply H. right. apply H.
  }
...
Proof.
  ...
  - intros. simpl. split.
    + right. apply H.
    + intros H. destruct H as [F | I].
      destruct F. apply I.
  - intros. simpl. split.
    + intros [Apa | Inall].
      -- left. left. apply Apa.
      -- apply IH in Inall. rewrite <- or_assoc. 
         right. apply Inall.
    + intros [[Apa | Inal'] | Inal0].
      -- left. apply Apa.
      -- right. apply IH. left. apply Inal'.
      -- right. apply IH. right. apply Inal0.
  Qed.
(** [] *)
...
Proof.
  intros T P l. split.
  {
    ...
    - intros H.
      ...
      + simpl in H.
        assert (H': (forall y, In y l -> P y)).
          { intros. specialize H with (x := y). apply H. right. apply H0. }
        apply IHl in H'.
        apply H'.
  }
  {
    ...
    - simpl in H. destruct H.
      intros y. simpl. intros [XisY | YinL].
      + rewrite <- XisY. apply H.
      + assert (H': forall x, In x l -> P x).
          { apply IHl. apply H0. }
...
Proof.
  intros.
  destruct (odd n) eqn:O.
  - unfold combine_odd_even. rewrite O. apply H. reflexivity.
  - unfold combine_odd_even. rewrite O. apply H0. reflexivity.
  Qed.
...
Proof.
  ...
  - destruct IHn.
    destruct (even n) eqn: E in H.
    + assert (H': even (S n) = false).
      { rewrite even_S. rewrite E. reflexivity. }
      rewrite H'.
      exists x. f_equal. apply H. 
    + assert (H': even (S n) = true).
      { rewrite H. simpl. apply even_double. }
      rewrite H'.
      exists (S x).
...
Proof.
  ...
  split.
  - destruct b1 eqn:B1.
    + simpl. intros. apply conj. reflexivity. apply H.
    + simpl. intros. discriminate H.
  - intros [B1 B2]. rewrite B1. simpl. apply B2.
  Qed.

Lemma orb_com : forall b1 b2, b1 || b2 = b2 || b1.
Proof.
  intros b1 b2.
  destruct b1, b2.
...
Proof.
  ...
  split.
  - destruct b1 eqn:B1.
    + simpl. intros. left. apply H.
    + simpl. intros. right. apply H.
  - intros [B1 | B2].
    + rewrite B1. reflexivity.
    + rewrite B2. rewrite orb_com. reflexivity.
  Qed.
(** [] *)
...
Proof.
  ...
  - intros.
    ...
    unfold not. intros.
    rewrite eqb_eq in H0. apply H in H0.
    apply H0.
  Qed.
(** [] *)
...
Proof.
  ...
  - split.
    {
      intros. simpl in H0. rewrite andb_true_iff in H0. destruct H0.
      rewrite (H x x0) in H0. rewrite H0.
      rewrite (IHl1 l2) in H1. rewrite H1.
      reflexivity.
    }
    {
      intros. injection H0 as H0.
      simpl. rewrite andb_true_iff. apply conj.
      apply (H x x0) in H0. apply H0.
      apply (IHl1 l2) in H1. apply H1.
    }
  Qed.
...
Lemma rev_append_appends: forall X xs ys, @rev_append X xs ys = (@rev_append X xs []) ++ ys.
Proof.
  induction xs.
  - reflexivity.
  - intros. simpl.
...
Theorem lem_to_ito: excluded_middle -> implies_to_or.
Proof.
