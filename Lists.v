...
Definition neqb (n m: nat): bool := negb (n =? m).
Fixpoint nonzeros (l:natlist) : natlist :=
  match l with
  | [] => []
  | x :: xs => if x =? 0 then nonzeros xs else x :: nonzeros xs
  end.
...
Fixpoint count (v : nat) (s : bag) : nat :=
  match s with
  | [] => 0
  | x :: xs => if v =? x then S (count v xs) else count v xs
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | [] => []
  | x :: xs => if v =? x then remove_all v xs else x :: remove_all v xs
  end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | [] => true
  | x :: xs => if (count x s1) <=? (count x s2) then included xs s2 else false
  end.
...
Theorem bag_count_sum : forall v b1 b2, count v (sum b1 b2) = count v b1 + count v b2.
Proof.
  intros.
  induction b1.
  - reflexivity.
